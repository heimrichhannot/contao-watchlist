...
class WatchlistItemEnclosure extends WatchlistItemDownload implements WatchlistItemViewInterface
{
    ...
    public static function generateCustomAddActionWithWatchlistSelect($arrData, $id)
    {
        global $objPage;
        ...
        $select = WatchlistController::getAllWatchlists();
//        $user       = FrontendUser::getInstance();
        ...
        $objT->addHref  = \HeimrichHannot\Ajax\AjaxAction::generateUrl(\HeimrichHannot\Watchlist\Controller\WatchlistController::XHR_GROUP, \HeimrichHannot\Watchlist\Controller\WatchlistController::XHR_WATCHLIST_ADD_ACTION, ['id' => $strUuid, 'cid' => $arrData['id'], 'type' => $arrData['type'], 'pageID' => $objPage->id]);
        $objT->addTitle = $GLOBALS['TL_LANG']['WATCHLIST']['addTitle'];
        $objT->addLink  = $GLOBALS['TL_LANG']['WATCHLIST']['addLink'];
        $objT->active   = true;
        $objT->id       = $strUuid;
